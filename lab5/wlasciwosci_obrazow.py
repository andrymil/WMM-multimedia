...
image = cv2.imread("football_mono.png", cv2.IMREAD_UNCHANGED)
printi(image, "image")
cv_imshow(image, "image")
cv2.waitKey(0)  ### oczekiwanie na reakcję użytkownika - naciśnięcie klawisza lub zamknięcie okien
...
"""
bitrate = 8*os.stat("football_mono.png").st_size/(image.shape[0]*image.shape[1])
...
image_col = cv2.imread("football_col.png")
...
image = cv2.imread("football_col.png", cv2.IMREAD_UNCHANGED)
